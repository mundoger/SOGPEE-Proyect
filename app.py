...
def agregar():
    ...
    return render_template('/Agregar.html',cargar = opcion, asesorAcademic = asesorAcademico)#Se redirige a la pagina de agregar proyecto y se mandan los datos de los asesores
#Funcion similar a la anterior pero usada en otra pagina alternativa
@app.route('/asignarEquipo',methods=['POST'])
def asignarEquipo():
    opcion = cargarAsesorEmp()
    asesorAcademico = cargarAsesorAcademico()
    return render_template('/Agregar.html',cargar = opcion, asesorAcademic = asesorAcademico)
#Funcion que carga la pagina de agregar proyecto cuando se borra un proyecto (alternativa)
...
def visualizarPDF(ruta):
    ...
    try:
        ...
        return url_for('static', filename=f"temp/{image_filename}"), None
    except Exception as e:
